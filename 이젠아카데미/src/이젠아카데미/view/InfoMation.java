...
public class InfoMation {
	...
	public void InfoMationMenu() {
		System.out.println("1.자유 게시판 2.출석하기 3. 출석률 4.뒤로가기"); int ch =sc.nextInt();
		// 기준 수정

		if(ch==1) {board();}
		if(ch==2) {attendAction();}
		if(ch==3) {attendancePercent();}
		if(ch==4) {}
		//if(ch==5) {훈련장려금();}
		/* if(ch==6) {} */
	...
	public void attendAction() {
		boolean result = AttendanceController.getInstance().attendAction();
		if(result) {System.out.println(" 출석 완료 "); InfoMationMenu();}
		else {System.out.println("이미 출석 했습니다."); InfoMationMenu();}
	}
	public void attendancePercent() {
	      int 출석횟수 = AttendanceController.getInstance().checkInCount();
	      System.out.println("나의 출석 횟수 : " + 출석횟수);
	      int 총수강일 = AttendanceController.getInstance().totalDaysAttended();
